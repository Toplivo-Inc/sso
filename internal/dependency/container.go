...
func MustBuild() *Dependencies {
	...
	sessionRepo := storage.NewSessionRepo(d.DB)
	clientRepo := storage.NewClientRepo(d.DB)
	authRepo := storage.NewAuthRepo(d.DB)

	// Set up services
	userService := service.NewUseService(userRepo, sessionRepo, clientRepo)
	oauthService := service.NewOAuthService(clientRepo, authRepo)
	clientService := service.NewClientService(clientRepo)
	d.sessionService = service.NewSessionService(sessionRepo, userRepo)
